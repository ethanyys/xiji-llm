...
def metric(y_true, y_pred, label2id):
    ...
    tot_precision = nb_correct / nb_pred if nb_pred > 0 else 0
    tot_recall = nb_correct / nb_true if nb_true > 0 else 0
    tot_score = 2 * tot_precision * tot_recall / (tot_precision + tot_recall) if tot_precision + tot_recall > 0 else 0
    ...
    for label in label2id:
        ...
        for t in true_entities:
            if t[0] == label: true_entities_label.add(t)
        for p in pred_entities:
            if p[0] == label: pred_entities_label.add(p)
        ...
        precision[label] = p_label
        recall[label] = r_label

    return tot_precision, tot_recall, tot_score, \
